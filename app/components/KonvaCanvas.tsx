...
export const KonvaCanvas = forwardRef<CanvasRef, KonvaCanvasProps>(
  (
    {
      sessionId,
      color,
    ...
    }, [getPointerPosition, updateUserPresence, isP2PConnected, dimensions, sendStrokePoint])
    ...
    }, [isDrawing, currentStroke, getPointerPosition, updateUserPresence, isP2PConnected, currentStrokeId, dimensions, sendStrokePoint, sendCursorPosition])

    // Handle pointer up
    const handlePointerUp = useCallback((e: Konva.KonvaEventObject<PointerEvent>) => {
      if (!isDrawing || strokeEndedRef.current) return
      ...
      onStrokeEnd?.()
    }, [isDrawing, currentStroke, getPointerPosition, color, size, opacity, addStrokeToSession, onStrokeEnd])
          ...
          {layers
            ...
            .map((layer) => {
              ...
              if (layer.type === 'stroke') {
                return (
                  <Layer key={layer.id} ref={strokeLayerRef} listening={false} opacity={layer.opacity}>
              ...
              if (layer.type === 'image') {
                ...
                return (
                  <Layer key={layer.id} ref={layer.id === images?.[0]?._id ? imageLayerRef : undefined} listening={false} opacity={layer.opacity}>
                    <KonvaImage
                      image={loadedImage}
                      x={image.x}
                      y={image.y}
              ...
              if (layer.type === 'ai-image') {
                ...
                return (
                  <Layer key={layer.id} ref={layer.id === aiImages?.[0]?._id ? aiImageLayerRef : undefined} listening={false} opacity={layer.opacity}>
                    <KonvaImage
                      image={loadedImage}
                      x={aiImage.x}
                      y={aiImage.y}
